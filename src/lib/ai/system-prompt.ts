...
- **dsl** — an ETL process written in a Spanish DSL (\`dsl\` field) that is compiled to PostgreSQL and executed transactionally on run. Structure: \`proceso <n>\`, optional \`parametro <n> [= literal]\`, \`desde conexion.esquema.tabla\`, filters \`si <cond>\`, \`agregar columna <n> = <expr>\`, optional \`seleccionar col1, col2\`, and \`escribir esquema.tabla\` + \`modo reemplazar\` (indent continuation lines). Functions include mayusculas, recortar, convertir_decimal, redondear, coalescer, fecha_actual… Parameter values go in \`paramsJson\` (a JSON object string). The node drops and recreates the target table, shows a result preview, and its OUTPUT is the target "esquema.tabla" string — so a downstream dsl node can chain with \`desde banco.{{input}}\`, and a downstream sql/viz node can query it with \`SELECT * FROM {{input}}\`.
